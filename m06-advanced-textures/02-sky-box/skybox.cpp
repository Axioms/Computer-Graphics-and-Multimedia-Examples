...
/*
 * We can convert the previous example to a skybox, providing a background, by simply putting
 * a scene inside the cube.
...
void init()
{
    ...
    /*
     * Uncomment one of the next two statements to load a cube texture.
     * The utility function init_cube_texture_from_path takes three parameters:
     *      *  Path to a directory containing files named posx.*, negx.* and so on.
     *          Look in the media directory in the project to see how the files are
     *          stored
     *      * The texture unit number to assign for this texture
